...
def get_abstract_model(model:keras.models.Model, 
                       n_class:int=10, 
                       final_affine:bool=False, 
                       final_ibp:bool=True)->keras.models.Model:

    C: Tensor = Input((n_class, n_class - 1))
    # (batch_size, n_class, n_class-1)

    decomon_model: keras.model.Model = clone(
            model, final_affine=final_affine, final_ibp=final_ibp, final_lower=False, backward_bounds=[C]
