...
def get_greedy(index_knapsack:np.ndarray,
    ...
    i_max:np.ndarray = np.argsort(np.max(W, 2)) # consider the worst case impact across all outputs ()
    n_g:int = len(index_knapsack)
    order:np.ndarray = np.max(np.cumsum(W[np.arange(n_g)[:,None], i_max], axis=1) + bias[:,None], -1) #(n_g, n_in)
    #threshold:np.ndarray = np.where(order<=0)

    for p, k in enumerate(index_knapsack):
        threshold_k:np.ndarray = np.where(order[p]<=0)[0][-1]
        coeff_k:list[int]= [i for i in i_max[p, :threshold_k] if i not in xai_indices and i not in free_indices][:card_knapsack[p]]
