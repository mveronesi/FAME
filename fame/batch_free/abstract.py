...
from .utils import encode_matrix


def get_abstract_output_domain_singleton(model:keras.models.Model,
    ...
    batch_size:int = n_in_wo_channel - len(xai_indices)-len(free_indices) # number of remaining indices
    gt_label:int = model.predict(input_sample[None], verbose=0).argmax(-1)[0]

    # repeat subdomains
    # freeze xai features to the nominal value
    lower_bound_np:np.ndarray = np.copy(input_sample_c) +0. # (channel, n_in_wo_channel) or (n_in_wo_channel, channel)
    upper_bound_np:np.ndarray = np.copy(input_sample_c) +0.
    if len(free_indices):
        if data_format=='channels_first':
            lower_bound_np[:,free_indices]=lower_bound_c[:, free_indices] # open the free dimension
    ...
    # repeat
    lower_bound_batch:np.ndarray = np.repeat(lower_bound_np[None], repeats=batch_size, axis=0) # (batch_size, channel, n_in_wo_channel)
    upper_bound_batch:np.ndarray = np.repeat(upper_bound_np[None], repeats=batch_size, axis=0) # (batch_size, channel, n_in_wo_channel)
    ...
    assert len(remaining_indices)==batch_size, 'Value Error remaining indices length should match batch_size'
    ...
    lower_bound_batch = np.reshape(lower_bound_batch, (-1, 1, n_in_with_channel)) #(batch_size, 1, n_in_with_channel)
    upper_bound_batch = np.reshape(upper_bound_batch, (-1, 1, n_in_with_channel)) #(batch_size, 1, n_in_with_channel)
    box:np.ndarray = np.concatenate([lower_bound_batch,
                                     upper_bound_batch
                                    ], 1) # (batch_size, 2, n_in_with_channel)


    # build your input domain
    # encode matrix C
    C:Tensor = Input((n_class, n_class-1))
    C_gt:np.ndarray = np.repeat(encode_matrix(n_class=n_class, groundtruth=gt_label)[None], repeats=batch_size, axis=0)
    # (batch_size, n_class, n_class-1)

    decomon_model:keras.model.Model = clone(model,
                                            final_affine=False, final_ibp=True, final_lower=False,
