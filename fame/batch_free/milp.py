...
def get_milp_sample(W:np.ndarray, b:np.ndarray, xai_indices:list[int], free_indices:list[int], cardinality:int)->list[int]:

    card_opt, solution_opt = solve_max_cardinality_milp(W.T, b)
    if card_opt:
        coeff_opt = [i for i in np.where(solution_opt==1)[0] if i not in xai_indices and i not in free_indices][:cardinality]
    else:
        coeff_opt = []

    return coeff_opt

def get_milp(index_knapsack:np.ndarray, card_knapsack:list[int],
               W:np.ndarray, b:np.ndarray,
               xai_indices:list[int],
               free_indices:list[int],
               abstract_free_set:np.ndarray)->np.ndarray:

    for p, k in enumerate(index_knapsack):
        coeff_k = get_milp_sample(W=W[p], b=b[p],
